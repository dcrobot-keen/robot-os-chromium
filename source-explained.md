...
이후 실제 크롬+실제 게임패드 테스트에서 같은 값이 50ms가 아니라 500ms 이상 간격으로만 로그에 찍히는 현상이 나왔는데(`plan.md` "실사용 테스트 — 게임패드 조작 확인" 참고), Node 테스트로는 재현이 안 됐다. 크롬이 `requestAnimationFrame` 없이 `setInterval`만으로 폴링하면 게임패드 상태 갱신 자체가 렌더링 프레임에 묶여 느려지는 알려진 동작이 있어서 이게 원인으로 의심되지만 아직 확인 전이다. 그래서 `onTick(pad)` 콜백을 추가했다 — 매 폴링마다(게임패드가 없어도) 호출되어서, 실제로 `_tick()`이 몇 번이나 도는지와 매번 게임패드가 잡히는지를 대시보드가 직접 셀 수 있게 했다. 추측 대신 실측하려는 목적이다.
...
두 번째 실사용 테스트(실제 게임패드로 조작)에서는 조작 자체는 잘 됐지만, `TeleopNode`가 50ms마다 무조건 발행하도록 짜여 있는데도 같은 값이 500ms 넘는 간격으로만 로그에 찍히는 현상이 나왔다. 이걸 진단하려고 `teleop ticks/s`, `gamepad seen/s`, `raw axes` 세 개를 화면에 실시간으로 띄우는 카운터를 추가했다(1초마다 리셋). `teleop-node.js`의 `onTick` 콜백이 매 폴링마다 불리는 걸 그대로 세는 것뿐이라, 이 숫자가 낮게 나오면 `_tick()` 자체가 안 도는 것이고, 숫자는 정상인데 `raw axes`가 안 바뀌면 크롬의 게임패드 상태 갱신이 문제라는 걸 구분할 수 있다.
