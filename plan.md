...
다만 로그에 이상한 점이 있다. `TeleopNode`는 50ms마다 무조건 버스에 발행하도록 짜여 있어서 스틱이 중립일 때도 `left=0 right=0`이 계속 찍혀야 하는데 그런 줄이 하나도 없고, 같은 값(0.6/-0.1)이 두 번 찍힌 간격도 570ms로 50ms보다 훨씬 길다. 이게 붙여넣은 로그가 일부만 발췌된 것인지, 아니면 폴링이 설계와 다르게 동작하고 있는 것인지는 아직 확인하지 못했다.
