...
macOS 크롬에서 실제 게임패드로 조작한 로그를 받았다.
...
속도 값이 스틱 조작에 따라 실제로 바뀌었고(0.6/-0.1 → -0.5/-0.5), 탭을 닫은 뒤 240ms 만에 ESTOP이 독자적으로 발동해서 안전 모델도 이 페이지에서 다시 확인됐다 — Phase 3의 게임패드 통과 기준을 처음 실증했다.

다만 로그에 이상한 점이 있다. `TeleopNode`는 50ms마다 무조건 버스에 발행하도록 짜여 있어서 스틱이 중립일 때도 `left=0 right=0`이 계속 찍혀야 하는데 그런 줄이 하나도 없고, 같은 값(0.6/-0.1)이 두 번 찍힌 간격도 570ms로 50ms보다 훨씬 길다. 붙여넣은 로그가 터미널에 찍힌 그대로라는 걸 확인받아서, 일부 발췌 가능성은 배제됐다.

### 원인 조사

`navigator.getGamepads()`를 목(mock)으로 바꿔서 `TeleopNode`의 틱 빈도를 Node로 직접 재봤다. 50ms 인터벌 기준 1초에 약 15번(기대치 20번과 비슷한 범위)이 나와서, 인터벌/발행 로직 자체는 문제없다는 걸 확인했다 — 즉 버그가 있다면 이 코드 안이 아니라 실제 크롬의 게임패드 상태 갱신 쪽일 가능성이 높다. 크롬은 `requestAnimationFrame` 없이 `setInterval`만으로 게임패드를 폴링하면 내부 상태 갱신이 렌더링 프레임에 묶여 예상보다 드물어지는 알려진 동작이 있는데, 이게 원인으로 의심되지만 아직 실측하지 않았다.

추측 대신 실측하기 위해 `TeleopNode`에 `onTick(pad)` 콜백을 추가했다 — 게임패드가 없어도 매 폴링마다 호출된다. 대시보드에는 1초마다 리셋되는 `teleop ticks/s`, `gamepad seen/s`, 그리고 매 틱의 원시 축 값(`raw axes`)을 보여주는 카운터를 추가했다. 다음 테스트에서 이 세 숫자를 보면 원인을 좁힐 수 있다 — ticks/s가 낮으면 `_tick()` 자체가 덜 도는 것(타이머 스로틀링), ticks/s는 정상인데 raw axes가 안 바뀌면 크롬의 게임패드 상태 갱신 문제로 구분된다.
